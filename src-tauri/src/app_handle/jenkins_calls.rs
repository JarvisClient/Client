...
#[tauri::command]
pub async fn stop_build(baseurl: String, username: String, apitoken: String, project_name: String, build_number: &str) -> Result<String, String> {
    let jenkins_client = JenkinsClient::new(&baseurl, &username, &apitoken);
    match jenkins_client.stop_build(&project_name, build_number).await {
        Ok(data) => Ok(data),
        Err(err) => Err(err.to_string()),
    }
}
