...
import { JenkinsInfo } from "./IJenkinsInterface";
...
const Modal: React.FC<ModalProps> = ({ isOpen, closeModal, modalData }) => {
    ...
    return isOpen ? (
        <motion.div
            ...
            onClick={() => closeModal()}>
            <div className="bg-background-sidebar p-14 w-[850px] h-[600px] rounded-md relative overflow-y-scroll overflow-x-hidden custom-scroll scrollbar-hidden"
            onClick={(e) => e.stopPropagation()}>
                ...
                <div className="flex flex-row mb-4">
						<div className="flex-shrink-0 mr-3 ">
							<span className="relative flex h-[37px] w-[37px]">
								...
								<span className={"relative inline-flex rounded-full h-full w-full bg-jenkins-job-" + (modalData["color"] == "notbuilt" ? "gray" : modalData["color"]) }></span>
							</span>
						</div>
                ...
                <div className="grid grid-cols-[100px,auto] mt-8 ">
                    ...
                    <div className="flex flex-col pt-2">
                        ...
                        <div className="ml-4">
                            <p><b>Last Build: </b> {modalData["lastBuild"] ? <a href={modalData["lastBuild"]["url"]}>#{modalData["lastBuild"]["number"]}</a> : 'None'}</p>
                            <p><b>Last Successful Build: </b> {modalData["lastSuccessfulBuild"] ? <a href={modalData["lastSuccessfulBuild"]["url"]}>#{modalData["lastSuccessfulBuild"]["number"]}</a> : 'None'}</p>
                            <p><b>Last Failed Build: </b> {modalData["lastFailedBuild"] ? <a href={modalData["lastFailedBuild"]["url"]}>#{modalData["lastFailedBuild"]["number"]}</a> : 'None'}</p>
                ...
                {modalData["healthReport"].length !== 0 ? (
                    <div className="grid grid-cols-[100px,auto] mt-8 ">
                        ...
                        <div className="flex flex-col pt-2">
                            ...
                            <div className="ml-4">
                                <p><b>Build stability: </b>{modalData["healthReport"][0]["description"].split(": ")[1]}</p>
                                <p><b>Score: </b>{modalData["healthReport"][0]["score"]}%</p>
                            </div>
                        </div>
                    </div>
                ) : null}
