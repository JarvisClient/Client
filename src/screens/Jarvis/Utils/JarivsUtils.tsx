...
export class JarvisUtils {
	...
	createJobCardProps = async (builds: IJenkinsProjectBuild[]) => {
		...
		try {
			if (!this.storedProjectName) throw new Error("No project name found in StorageManager");
			const jobCardProps = await Promise.all(builds.map(async (build: IJenkinsProjectBuild) => {
				...
				const projectName = this.storedProjectName as any;
				return {
					buildNumber: build.number,
					displayName: details.displayName,
					description: details.description,
					result: details.result,
					pinned: (pinnedJobs[projectName] || []).includes(String(build.number)),
					notification_set: (notificationSetJobs[projectName] || []).includes(String(build.number)),
	...
	updatePinnedJobCards = (jobCardProps: JobCardProps[]) => {
		try {
			...
			const updatedJobCardProps = jobCardProps.map((element: JobCardProps) => ({
				...element,
				pinned: pinnedJobs[this.storedProjectName as any].includes(String(element.buildNumber)),
			}));
	...
	updateNotificationSetJobCards = (jobCardProps: JobCardProps[]) => {
		try {
			...
			const updatedJobCardProps = jobCardProps.map((element: JobCardProps) => ({
				...element,
				pinned: notificationSetJobs[this.storedProjectName as any].includes(String(element.buildNumber)),
			}));
